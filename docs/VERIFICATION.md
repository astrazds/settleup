...
- `npm run build:client`: bundles the React Event page into `src/ui/generated-client.ts` and compiles shadcn/Tailwind styles into `src/ui/generated-shadcn-styles.ts`; it is also run by the relevant npm pre-scripts.
...
Transient generated output stays out of commits. `src/ui/generated-client.ts` and `src/ui/generated-shadcn-styles.ts` are checked-in app-served browser assets and are regenerated by `npm run build:client`.
